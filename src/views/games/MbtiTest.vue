...
const calculateResult = () => {
  ...
  const subType = subTypes[Math.floor(seededRandom(Date.now()) * subTypes.length)]
...
const share = async () => {
  ...
  const text = `${t('tools.mbti.shareText')} ${result.value.type}! ${t(`tools.mbti.types.${result.value.type}.tagline`)}`
